    ...
    <div className="fade-up" style={{ animationDelay: '0ms', margin: '14px 18px 0', borderRadius: 24, overflow: 'hidden', position: 'relative', minHeight: 196, background: '#2E4A3A', boxShadow: '0 1px 2px rgba(20,18,12,0.06), 0 18px 36px rgba(20,18,12,0.14)' }}>
      ...
      <div style={{ position: 'relative', zIndex: 2, color: '#F5F1E8', padding: '18px 20px 18px', display: 'flex', flexDirection: 'column', minHeight: 196 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <Eyebrow color="rgba(245,241,232,0.78)">{new Date().toLocaleDateString('en-IN', { weekday: 'short', day: 'numeric', month: 'short' })} · Hardoi</Eyebrow>
        ...
        <div style={{ marginTop: 'auto' }}>
          <h1 style={{ fontFamily: 'Fraunces', fontWeight: 500, fontSize: 36, lineHeight: 1.02, letterSpacing: '-0.025em', color: '#FAF6EC', margin: 0, fontVariationSettings: '"opsz" 60', textShadow: '0 2px 18px rgba(20,18,12,0.35)' }}>
            Good morning,<br /><span style={{ fontStyle: 'italic', fontWeight: 400 }}>{name}.</span>
          </h1>
        </div>
      </div>
    </div>
...
function StatsRibbon({ stops, urgentCount }: { stops: number; urgentCount: number }) {
  const stats = [
    { k: 'Visits', v: String(stops), sub: 'planned' },
    { k: 'Urgent', v: String(urgentCount), sub: 'high priority' },
    { k: 'Today', v: new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'short' }), sub: '' },
  ]
  return (
      ...
      {stats.map((s, i) => (
        <React.Fragment key={s.k}>
          <div style={{ textAlign: i === 1 ? 'center' : (i === 2 ? 'right' : 'left') }}>
            ...
            <div style={{ fontFamily: 'Plus Jakarta Sans', fontSize: 10.5, fontWeight: 600, color: 'var(--ink-soft)', letterSpacing: '0.1em', textTransform: 'uppercase' as const, marginTop: 4 }}>{s.k}{s.sub && <> · <span style={{ fontWeight: 500, letterSpacing: '0.04em', textTransform: 'none' as const, opacity: 0.85 }}>{s.sub}</span></>}</div>
          </div>
...
function AlertCard({ alert }: { alert: { title: string; description: string; time: string } | null }) {
  const title = alert?.title || 'Wheat blight risk rising in 3 farms'
  const desc = alert?.description || '48mm rainfall + humidity spike'
  const time = alert?.time || '2h ago'
  return (
    <div className="fade-up" style={{ animationDelay: '140ms', margin: '0 18px 24px', borderRadius: 20, background: 'var(--surface)', boxShadow: '0 1px 2px rgba(20,18,12,0.04), 0 10px 28px rgba(20,18,12,0.07)', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', top: 12, right: 12, width: 64, height: 64, borderRadius: 14, backgroundImage: `url(${PHOTO_WHEAT})`, backgroundSize: 'cover', backgroundPosition: 'center', border: '1px solid var(--border)' }} />
      <div style={{ padding: '16px 18px 16px 22px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
          <PulseDot color="#B85C3C" size={6} />
          <Eyebrow color="var(--danger)">Predictive Alert</Eyebrow>
        </div>
        <h3 style={{ fontFamily: 'Fraunces', fontWeight: 500, fontSize: 21, lineHeight: 1.14, letterSpacing: '-0.015em', color: 'var(--ink)', margin: 0, maxWidth: 240, fontVariationSettings: '"opsz" 30' }}>{title}</h3>
        <p style={{ fontFamily: 'Plus Jakarta Sans', fontSize: 13, color: 'var(--ink-soft)', margin: '8px 0 0', lineHeight: 1.45 }}>
          {desc} · <span style={{ color: 'var(--ink)' }}>{time}</span>
        </p>
...
function SectionHeader({ urgentCount }: { urgentCount: number }) {
  return (
    <div className="fade-up" style={{ animationDelay: '210ms', padding: '0 22px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
      ...
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 11px', borderRadius: 999, background: 'rgba(201,151,74,0.18)', color: '#8C6420', fontFamily: 'Plus Jakarta Sans', fontWeight: 700, fontSize: 11, letterSpacing: '0.04em' }}>{urgentCount} urgent</span>
    </div>
  )
}
...
interface VisitItem {
  name: string; initials: string; risk: 'HIGH' | 'MEDIUM' | 'LOW'
  confidence: number; village: string; dist: string; crop: string
  reason: string; product: string; photo: string; type: 'retailer' | 'farmer'
}

function VisitCard({ farmer, index }: { farmer: VisitItem; index: number }) {
...
const MOCK_VISITS: VisitItem[] = [
  { name: 'Kisan Agri Store', initials: 'KA', risk: 'HIGH', confidence: 96, village: 'Sandila', dist: '2.1', crop: 'Topik OOS', reason: 'Topik 15WP out of stock — 4 farmers at heading stage need restock urgently.', product: 'Topik 15WP', photo: '', type: 'retailer' },
  { name: 'Ramesh Yadav', initials: 'RY', risk: 'HIGH', confidence: 94, village: 'Bhatpura', dist: '4.2', crop: 'Wheat · 3.2 ac', reason: 'Blight risk after 48mm rain at flowering — foliar window closes in 36h.', product: 'Tilt 25EC', photo: 'https://images.unsplash.com/photo-1605000797499-95a51c5269ae?w=200&q=80&auto=format&fit=crop', type: 'farmer' },
  { name: 'Suresh Verma', initials: 'SV', risk: 'HIGH', confidence: 91, village: 'Mallawan', dist: '6.8', crop: 'Wheat · 2.4 ac', reason: 'Yellow rust on neighbouring plots; humidity favours spread by Thursday.', product: 'Nativo 75WG', photo: 'https://images.unsplash.com/photo-1531123897727-8f129e1688ce?w=200&q=80&auto=format&fit=crop', type: 'farmer' },
  { name: 'Sharma Seeds', initials: 'SS', risk: 'MEDIUM', confidence: 82, village: 'Atrauli', dist: '8.3', crop: 'Score low', reason: 'Score 250EC at 4 units — drawdown rate suggests stockout by Friday.', product: 'Score 250EC', photo: '', type: 'retailer' },
  { name: 'Manju Devi', initials: 'MD', risk: 'MEDIUM', confidence: 76, village: 'Bhatpura', dist: '4.5', crop: 'Wheat · 1.8 ac', reason: 'Soil moisture above threshold; preventive note pending farmer consent.', product: 'Amistar Top', photo: 'https://images.unsplash.com/photo-1594744803329-e58b31de8bf5?w=200&q=80&auto=format&fit=crop', type: 'farmer' },
]

function mapRouteToVisit(stop: any, index: number): VisitItem {
  const priority = stop.priority === 'high' ? 'HIGH' : stop.priority === 'medium' ? 'MEDIUM' : 'LOW'
  return {
    name: stop.name || stop.retailer_id,
    initials: (stop.name || stop.retailer_id || 'R').slice(0, 2).toUpperCase(),
    risk: priority,
    confidence: priority === 'HIGH' ? 92 - index * 3 : priority === 'MEDIUM' ? 78 - index * 2 : 65,
    village: stop.tehsil || 'Hardoi',
    dist: String(((index + 1) * 2.1).toFixed(1)),
    crop: 'Retailer',
    reason: stop.pitch || stop.reason || `Last visited ${stop.days_since_last_visit || '14+'} days ago`,
    product: stop.pitch?.split('—')[0]?.trim() || 'Tilt 250 EC',
    photo: '',
    type: 'retailer',
  }
}
...
export default function MorningBriefing() {
  ...
  const [topAlert, setTopAlert] = useState<{ title: string; description: string; time: string } | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    Promise.all([
      apiFetch<{ route: any[]; total_stops: number }>(`/api/route?rep_id=${REP_ID}`).catch(() => null),
      apiFetch<{ alerts: any[] }>(`/api/alerts?rep_id=${REP_ID}`).catch(() => null),
      apiFetch<{ retailers: any[] }>(`/api/retailers?rep_id=${REP_ID}&limit=20`).catch(() => null),
    ]).then(([routeData, alertsData, retailerData]) => {
      if (routeData?.route?.length) {
        const retailerMap: Record<string, string> = {}
        if (retailerData?.retailers) {
          for (const r of retailerData.retailers) {
            retailerMap[r.retailer_id] = r.tehsil ? `${r.tehsil} Retailer` : r.retailer_id
          }
        }
        const mapped = routeData.route.map((stop, i) => mapRouteToVisit({
          ...stop,
          name: retailerMap[stop.retailer_id] || stop.retailer_id,
        }, i))
        setVisits(mapped)
      }

      if (alertsData?.alerts?.length) {
        const top = alertsData.alerts.find(a => a.severity === 'critical' || a.severity === 'high') || alertsData.alerts[0]
        setTopAlert({
          title: top.title,
          description: top.description || top.action || '',
          time: timeSince(top.created_at),
        })
      }
    }).finally(() => setLoading(false))
  }, [])

  const urgentCount = visits.filter(v => v.risk === 'HIGH').length
  ...
  return (
    <div style={{ position: 'relative', width: '100%', minHeight: '100%', background: 'var(--bg)', paddingTop: 48 }}>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        <TopStrip />
        <Hero name="Arjun" />
        <StatsRibbon stops={visits.length} urgentCount={urgentCount} />
        <AlertCard alert={topAlert} />
        <SectionHeader urgentCount={urgentCount} />
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, padding: '0 18px' }}>
          {visits.map((f, i) => <VisitCard key={`${f.name}-${i}`} farmer={f} index={i} />)}
        </div>
