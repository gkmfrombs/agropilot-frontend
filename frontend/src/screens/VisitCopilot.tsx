...
const talkingPoints = [
    'Wheat heading stage starts this week — ideal timing for Tilt 25EC application.',
    'Rainfall forecast suggests applying fungicide before Thursday rain event.',
    'Mustard aphid pressure increasing — recommend Actara 25WG for nearby potato growers.',
];
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {talkingPoints.map((tp, i) => (
                        <div key={i} style={{ display: 'flex', gap: 10, padding: '12px 14px', background: 'var(--surface-warm)', borderRadius: 12, border: '1px solid var(--border)' }}>
                            ...
                            <span style={{ fontFamily: 'Plus Jakarta Sans', fontSize: 13, color: 'var(--ink)', lineHeight: 1.45 }}>{tp}</span>
                        </div>
                    ))}
                </div>
            </div>
