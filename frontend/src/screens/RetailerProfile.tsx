...
import { IChev, INav, IStore, TopStrip, BottomNav, VoiceFAB } from '../components/Shared'
...
interface StockItem { sku: string; qty: number; status: 'OUT' | 'LOW' | 'OK'; history: number[] }
...
export default function RetailerProfile() {
  ...
  const [retailerName, setRetailerName] = useState('Kisan Agri Store')
  const [location, setLocation] = useState('Sandila Rd, Hardoi')
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    apiFetch<{ retailer_id: string; tehsil: string; district: string; inventory: any[] }>('/api/retailers/RTL_00001')
      .then(data => {
        if (data.tehsil) {
          setRetailerName(`${data.tehsil} Agri Store`)
          setLocation(`${data.tehsil}, ${data.district}`)
        }
        if (data.inventory?.length) {
          const mapped: StockItem[] = data.inventory.slice(0, 6).map((item: any) => {
            const qty = Number(item.sku_qty ?? 0)
            return {
              sku: item.sku_name || item.sku_id,
              qty,
              status: stockStatus(qty),
              history: [qty + 8, qty + 6, qty + 4, qty + 2, qty],
            }
          })
          setStock(mapped)
        }
  ...
  return (
    <div style={{ position: 'relative', width: '100%', minHeight: '100%', background: 'var(--bg)', paddingTop: 48 }}>
      ...
      <div style={{ padding: '14px 18px 20px', background: 'var(--surface)', borderBottom: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ fontFamily: 'Plus Jakarta Sans', fontSize: 12, color: 'var(--ink-soft)', marginTop: 2 }}>Best visit: <strong style={{ color: 'var(--ink)' }}>Tue/Thu mornings</strong></div>
          </div>
        </div>
      ...
      <div className="fade-up" style={{ padding: '20px 18px 0' }}>
        <h2 style={{ fontFamily: 'Fraunces', fontSize: 18, fontWeight: 500, color: 'var(--ink)', marginBottom: 14 }}>
          {t('currentStock')} {loading && <span style={{ fontSize: 12, color: 'var(--ink-soft)', fontFamily: 'Plus Jakarta Sans' }}>· {t('loading')}</span>}
        </h2>
            ...
            return (
              <div key={item.sku} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', background: 'var(--surface)', borderRadius: 14, border: '1px solid var(--border)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ padding: '3px 8px', borderRadius: 999, background: st.bg, color: st.fg, fontFamily: 'Plus Jakarta Sans', fontSize: 10, fontWeight: 700 }}>{item.status}</span>
                </div>
              </div>
            )
          })}
      ...
      <div className="fade-up" style={{ animationDelay: '100ms', padding: '20px 18px 0' }}>
        <h2 style={{ fontFamily: 'Fraunces', fontSize: 18, fontWeight: 500, color: 'var(--ink)', marginBottom: 14 }}>{t('competitorProducts')}</h2>
        {MOCK_COMPETITORS.map((c, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderBottom: i < MOCK_COMPETITORS.length - 1 ? '1px solid var(--border)' : 'none' }}>
            <div>
      ...
      <div className="fade-up" style={{ animationDelay: '200ms', padding: '20px 18px 0' }}>
        <h2 style={{ fontFamily: 'Fraunces', fontSize: 18, fontWeight: 500, color: 'var(--ink)', marginBottom: 14 }}>{t('nearbyFarmers')}</h2>
        {MOCK_FARMERS.map((f, i) => (
          <Link to="/farmer/1" key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderBottom: i < MOCK_FARMERS.length - 1 ? '1px solid var(--border)' : 'none', textDecoration: 'none', color: 'inherit' }}>
            <div>
