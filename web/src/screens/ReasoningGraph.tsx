...
const NODES = [
  { id: 'farmer',  label: 'Ramesh',       NodeIcon: IUser,      edge: 'treats',       angle: -90,  type: 'Person',               facts: [['Member', 'Since 2021'], ['Village', 'Bhatpura, Block 4'], ['Last visit', '7 days ago']], source: 'Farmer profile · CRM', weight: 18 },
  { id: 'variety', label: 'HD-2967',      NodeIcon: IWheat,     edge: 'for',          angle: -30,  type: 'Crop Variety',          facts: [['Susceptibility', 'Moderate to Septoria'], ['Plot size', '3.2 acres'], ['Stage', 'Flowering (BBCH 65)']], source: 'IARI cultivar database · synced 12h ago', weight: 22 },
  { id: 'rain',    label: '48mm rain',    NodeIcon: ICloudRain, edge: 'triggers',     angle:  30,  type: 'Environmental Signal',  facts: [['Window', 'Mon–Tue, 31h saturation'], ['Source', 'Hardoi block stations'], ['Threshold', 'Exceeded by 12mm']], source: 'IMD station feed · synced 6h ago', weight: 28 },
  { id: 'humid',   label: '89% humidity', NodeIcon: IDroplets,  edge: 'amplifies',    angle:  90,  type: 'Environmental Signal',  facts: [['Duration', '18 hours sustained'], ['Canopy wetness', 'Dawn window matched'], ['Spore release', 'Elevated']], source: 'IMD station feed · synced 6h ago', weight: 16 },
  { id: 'history', label: '2023 outbreak',NodeIcon: IHistory,   edge: 'matches',      angle:  150, type: 'Historical Match',      facts: [['Panchayat', 'Bhatpura + Mallawan'], ['Loss', '12–22% yield, 14 plots'], ['Pattern overlap', '0.87 cosine']], source: 'Field outbreak ledger · 2023 season data', weight: 12 },
  { id: 'stock',   label: 'Stock 5km',    NodeIcon: IPin,       edge: 'available at', angle:  210, type: 'Logistics Signal',      facts: [['Outlet', 'Kisan Store, Sandila Rd'], ['SKUs', 'Tilt 25EC · 500ml × 14'], ['Drive time', '11 min']], source: 'Retailer inventory · confirmed 2h ago', weight: 14 },
]

const STEPS = [
  { n: 1, text: 'Identified HD-2967 wheat at BBCH 65 (flowering) — highest fungal vulnerability window for this variety', src: 'Farmer profile · CRM' },
  { n: 2, text: '48mm rainfall over 31h + 89% humidity matched Septoria tritici infection conditions (Hardoi block IMD)', src: 'IMD station feed' },
  { n: 3, text: '0.87 cosine match to May 2023 outbreak — 14 affected plots, 12–22% yield loss in Bhatpura-Mallawan cluster', src: 'Field outbreak ledger' },
  { n: 4, text: 'Ranked fungicides by early-stage blight efficacy — Tilt 25EC highest label rate match for HD-2967 at flowering', src: 'Product catalogue' },
  { n: 5, text: 'Located 14 units Tilt 25EC at Kisan Store, Sandila Rd — 11 min drive, confirmed 2h ago via retailer sync', src: 'Retailer inventory · 2h ago' },
]
...
function GraphCanvas({ selected, onSelect, onCenterClick }: { selected: string | null; onSelect: (id: string) => void; onCenterClick: () => void }) {
  const edges = NODES.map((n) => {
    const p = polar(n.angle, RADIUS)
    const from = offsetAlong(p, CENTER, 30)
    const to = offsetAlong(CENTER, p, 47)
    return { id: n.id, from, to, mid: midpoint(from, to), label: n.edge }
  })
          ...
          return (
            <g key={e.id} style={{ opacity: dim ? 0.18 : 1, transition: 'opacity 240ms ease' }}>
              ...
              <g transform={`translate(${e.mid.x}, ${e.mid.y})`} className="edge-label" style={{ animationDelay: `${550 + i * 180}ms` }}>
                ...
                <text textAnchor="middle" y="3.5" style={{ fontFamily: 'Plus Jakarta Sans', fontSize: 8.5, fontWeight: 600, fill: '#6B6A5F', letterSpacing: '0.04em' } as React.CSSProperties}>{e.label}</text>
              </g>
            </g>
          )
        })}
            ...
            >
              ...
              <foreignObject x={p.x - 30} y={p.y - 26} width="60" height="52">
                <div style={{ width: 60, height: 52, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 2, pointerEvents: 'none' }}>
                  <n.NodeIcon size={16} stroke="#2E4A3A" />
        ...
        <g className="node-in" style={{ animationDelay: '700ms', transformBox: 'fill-box' as any, transformOrigin: 'center', cursor: 'pointer' }} onClick={onCenterClick}>
          ...
          <foreignObject x={CENTER.x - 42} y={CENTER.y - 36} width="84" height="72">
            <div style={{ width: 84, height: 72, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3, pointerEvents: 'none' }}>
              ...
              <div style={{ fontFamily: 'Fraunces', fontWeight: 500, fontSize: 13, color: '#FAF6EC', textAlign: 'center', lineHeight: 1.05 }}>Tilt 25EC</div>
              <div style={{ fontFamily: 'Plus Jakarta Sans', fontSize: 8, fontWeight: 700, color: 'rgba(245,241,232,0.85)', textAlign: 'center', letterSpacing: '0.10em' }}>94% confidence</div>
            </div>
          </foreignObject>
        </g>
      </svg>
...
function NodeDetail({ node }: { node: typeof NODES[0] }) {
  return (
    <div className="fade-up" style={{ margin: '16px 18px', padding: '16px 18px', background: 'var(--surface)', borderRadius: 20, border: '1px solid var(--border)', boxShadow: '0 4px 16px rgba(20,18,12,0.06)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
        <div style={{ width: 36, height: 36, borderRadius: 10, background: 'var(--primary-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <node.NodeIcon size={18} stroke="var(--primary)" />
        </div>
        <div>
...
export default function ReasoningGraph() {
  ...
  return (
    <div className="screen-root" style={{ position: 'relative', width: '100%', minHeight: '100%', background: 'var(--bg)' }}>
      ...
      <GraphCanvas selected={selected} onSelect={toggle} onCenterClick={() => setSelected(null)} />

      {/* Node detail or reasoning trail */}
      {selectedNode ? (
          ...
          {STEPS.map((s, i) => (
            <div key={s.n} className="fade-up" style={{ animationDelay: `${i * 80}ms`, display: 'flex', gap: 14, marginBottom: 14 }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 'none' }}>
                ...
                {i < STEPS.length - 1 && <div style={{ width: 1, flex: 1, background: 'var(--border)', marginTop: 4, marginBottom: 4 }} />}
              </div>
