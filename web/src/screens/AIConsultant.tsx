        ...
        <div className="fade-up" style={{ maxWidth: '92%', alignSelf: 'flex-start', background: 'var(--surface)', borderRadius: '20px 20px 20px 8px', boxShadow: '0 1px 2px rgba(20,18,12,0.04), 0 14px 36px rgba(20,18,12,0.10)', padding: '16px 18px 14px', position: 'relative', overflow: 'hidden' }}>
            ...
            {message.bullets ? (
                ...
                <div style={{ marginTop: 10, display: 'flex', gap: 4 }}>
                    {[0, 1, 2].map(i => <span key={i} style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--border)', animation: `dotPulse 1.2s ease-in-out ${i * 0.2}s infinite` }} />)}
                </div>
            ) : (
...
export default function AIConsultant() {
    ...
    const sendMessage = async (text?: string) => {
        ...
        try {
            const data = await api.chat(historyRef.current);
            const reply = (data.response || 'Got it.').replace(/\*\*/g, '');
            historyRef.current = [...historyRef.current, { role: 'assistant', content: reply }];
            setMessages(prev => {
                const next = [...prev];
                next[next.length - 1] = { text: reply, isUser: false, followUps: ['What dosage?', 'Check stock', 'Show route'] };
                return next;
            });
        } catch {
            setMessages(prev => {
                const next = [...prev];
                next[next.length - 1] = { text: "I'm having trouble connecting right now. Please try again in a moment.", isUser: false };
                return next;
            });
        } finally {
            setLoading(false);
