...
function StatsRibbon({ totalStops, loading }: StatsRibbonProps) {
    // Route km is synthetic but proportional to real stop count from API
    const visitsVal = loading ? '—' : String(totalStops ?? '—')
    const routeVal = loading ? '—' : ((totalStops ?? 0) * 3.1).toFixed(1)

    const stats = [
        { k: 'Visits', v: visitsVal, sub: 'planned' },
        { k: 'Route', v: routeVal, sub: 'km' },
        // Target is a fixed business goal, not sourced from the route API
...
// Static fallback visits — used when the API is unavailable
...
/**
 * Maps a single stop from the route API response to the shape
 * that VisitCard expects. Confidence is synthetic based on priority tier.
 */
function mapStopToVisit(stop: any) {
    const priorityUpper = stop.priority.toUpperCase() as 'HIGH' | 'MEDIUM' | 'LOW'
    const confidence = priorityUpper === 'HIGH' ? 92 : priorityUpper === 'MEDIUM' ? 78 : 65
    // pitch format: "Product Name — reason text"
    const pitchParts = stop.pitch.split('—')
    const productName = pitchParts[0].trim()
    return {
        name: stop.retailer_id,
        initials: stop.retailer_id.slice(-2).toUpperCase(),
        risk: priorityUpper,
        confidence,
        village: stop.tehsil,
        // Synthetic distance proportional to stop order (no GPS data from API)
        dist: ((stop.stop_number * 2.3) + 1.2).toFixed(1),
        crop: productName,
        reason: stop.pitch,
        product: productName,
        photo: '',
...
export default function MorningBriefing() {
    ...
    useEffect(() => {
        ...
        // Existing side-effect calls — preserved unchanged
        api.getAlerts().catch(() => null)
        api.getRoute().catch(() => null)

        // Route fetch with rep_id to populate visits and stats ribbon
        setVisitsLoading(true)
        api.getRouteForRep(effectiveRepId)
            .then((data: any) => {
                setRouteData(data)
                if (Array.isArray(data?.route) && data.route.length > 0) {
                    setVisits(data.route.map(mapStopToVisit))
                }
            })
            .catch(() => {
                // API failed — keep FALLBACK_VISITS already set as initial state
            })
            .finally(() => {
    ...
    return (
        <div className="screen-root" style={{ position: 'relative', width: '100%', minHeight: '100%', background: 'var(--bg)' }} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
                <TopStrip />
                <Hero name={name} />
                <StatsRibbon totalStops={routeData?.total_stops ?? null} loading={visitsLoading} />
                <AlertCard />
                <SectionHeader />
                <FilterChips />
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14, padding: '0 18px' }}>
                    {visitsLoading ? (
                        /* Loading skeleton — plain grey text placeholder, no layout change */
                        <div style={{ padding: '24px 0', textAlign: 'center', fontFamily: 'Plus Jakarta Sans', fontSize: 13, color: 'var(--ink-soft)' }}>
                            Loading...
                        </div>
                    ) : (
